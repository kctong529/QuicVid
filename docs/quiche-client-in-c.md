# Quiche C Bindings: Establishing QUIC Connections

## Overview
After exploring quiche's Rust tools, the next step is using the C bindings to establish QUIC connections programmatically. Quiche provides a C API that allows integration with C/C++ applications.

## Building the C Library

### Generate the C Library
From your quiche directory:
...
# target/release/libquiche.so (Linux)
# target/release/libquiche.dylib (macOS)
# target/release/quiche.dll (Windows)
```

### Locate Header Files
The C header file is located at:
```bash
...
### Simple QUIC Client in C

Create a file `quiche_client.c`:
...
```bash
# Linux
export LD_LIBRARY_PATH=./quiche/target/release:$LD_LIBRARY_PATH

# macOS  
export DYLD_LIBRARY_PATH=./quiche/target/release:$DYLD_LIBRARY_PATH
```

## Running the C Client

1. **Start the Rust server** (from earlier, in the quiche directory):
```bash
...
Sending initial packet (1200 bytes) to 127.0.0.1:4433
Initial packet sent successfully!
Received 85 bytes
Processed 85 bytes
Sent 1200 bytes
Received 1200 bytes
Processed 1200 bytes
Sent 1200 bytes
Received 459 bytes
Processed 459 bytes
...
ls -la quiche/target/release/libquiche.*

# Verify symbol exports
nm -D quiche/target/release/libquiche.so | grep quiche_config_new
```
