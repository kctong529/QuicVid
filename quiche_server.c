...
void handle_connection(quiche_conn *conn, uint8_t *buf, ssize_t len,
...
int run_server(int port) {
    ...
    while (1) {
        ...
        if (len < 0) {
            ...
            if (conn) send_pending(conn, sock, out);
            usleep(1000);
            continue;
        }
        ...
        if (conn) {
            handle_connection(conn, buf, len, &peer, peer_len, &local, sock, out);
            if (quiche_conn_is_closed(conn)) conn = NULL;
        }
    }
